...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, desc_bert_path_or_handler, sub_bert_path_or_handler,
        ...
        if train_moment:
            ...
            # 如果是train how2 执行下面
            retrieval_file = "/root/hk_tmp_data/inference_how2_train_None_predictions_VCMR_SVMR_VR.json"
            video2idx_file = "/root/hk_tmp_data/vid2dur_idx.json"
            idx2video_file = "/root/hk_tmp_data/how2_idx2video.json"
    ...
    def __getitem__(self, index):
        ...
        # 有效 token 数量
        valid_tokens_nums = model_inputs["video_feat"][:].shape[0]
        gt_ed = model_inputs["st_ed_indices"][1]
        if gt_ed >= valid_tokens_nums:
            print("gt_ed >= valid_tokens_nums")
            print("error")
            gt_ed = valid_tokens_nums - 1
...
def start_end_collate(batch):
    ...
    # 检查how2数据是否在 gt 内, 如果是tvr 可以注释掉------------
    # for i in range(len(batch)):
    #     # gt_st = math.floor(batch[i]["meta"]["ts"][0] / 1.5)
    #     gt_st = math.floor(batch[i]["meta"]["ts"][1] / 1.5)
    #     valid_token_nums = batched_data["video_feat"][1][i].sum()
    #     if valid_token_nums <= gt_st :
    #         if gt_st - 1 == 39:
    #             pass
    #         else:
    #             print("error")
    #             print(batch[i]["meta"]["ts"])
    # ------------------------------------
