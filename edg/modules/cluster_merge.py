...
def compute_temporal_distance_matrix(n, token_mask=None):
    """
    Compute the temporal distance matrix.
    Args:
        n (int): length of the temporal sequence.
        token_mask (tensor): validity mask
    Returns:
        temporal_dist_matrix (Tensor[n, n]): pairwise temporal distances.
    """
    temporal_indices = torch.arange(n)
    temporal_dist_matrix = torch.abs(temporal_indices.unsqueeze(0) - temporal_indices.unsqueeze(1))
    temporal_dist_matrix = temporal_dist_matrix / n
    return temporal_dist_matrix
...
def cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None):
    ...
    with torch.no_grad():
        ...
        if token_mask is not None:
            ...
            temporal_dist_matrix = compute_temporal_distance_matrix(N).unsqueeze(0).repeat(B, 1, 1).float().to(device=x.device, dtype=x.dtype)
            dist_matrix = dist_matrix + 0.4 * temporal_dist_matrix
...
class CTM(nn.Module):
    ...
    def forward(self, token_dict):
        ...
        down_dict["cluster_idx"] = index_down
