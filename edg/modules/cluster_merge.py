...
def compute_temporal_distance_matrix(n, token_mask=None):
    ...
    参数:
        n (int): 时间序列的长度
        token_mask (tensor): 掩码
    返回:
        temporal_dist_matrix (Tensor[n, n]): 时序距离矩阵
    """
    # token_len = token_mask.sum(dim=1)
    ...
    # temporal_dist_matrix = temporal_dist_matrix.masked_fill(~token_mask.unsqueeze(2), 0)
...
class TokenConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # 被我注释 x = x + self.conv(x.permute(0, 2, 1)).permute(0, 2, 1)
