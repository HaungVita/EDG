...
def batch_video_query_loss(video, query, match_labels, mask, measure='JSD'):
    ...
    E_pos = torch.sum(E_pos * pos_mask, dim=1) / (
        torch.sum(pos_mask, dim=1) + 1e-12)  # (bsz, )
    ...
    E_neg = torch.sum(E_neg * neg_mask, dim=1) / (
        torch.sum(neg_mask, dim=1) + 1e-12)  # (bsz, )

    E = E_neg - E_pos  # (bsz, )
    return torch.mean(E)
def hard_video_query_loss(video, query, match_labels, mask, measure='JSD'):
    qbsz, sample_num, lv, dim = video.shape
    # generate mask
    pos_mask = match_labels.type(torch.float32)  # (bsz, Lv)
    pos_padding = (0, lv * (sample_num - 1))  # (左, 右) 这里我们只在右边填充
    pos_mask = F.pad(pos_mask, pos_padding, "constant", 0)
    mask = mask.contiguous().view(qbsz, sample_num * lv)  # (bsz, sample_num * lv)
    neg_mask = (torch.ones_like(pos_mask) - pos_mask) * mask  # (bsz, Lv)

    # video: (bsz, sample_num, lv, dim) -> (bsz * sample_num, lv, dim)
    video = video.contiguous().view(qbsz * sample_num, lv, dim)
    # 每个query复制sample_num次
    query = query.unsqueeze(1).repeat(1, sample_num, 1).view(qbsz * sample_num, dim)  # (bsz * sample_num, dim)
    # query: (bsz, dim) -> (bsz, )
    res = torch.einsum("bld, bd -> bl", video, query)
    res = res.contiguous().view(qbsz, sample_num * lv)  # (bsz, sample_num, lv)
    E_neg = get_negative_expectation(res * neg_mask, measure, average=False)
    E_neg = torch.sum(E_neg * neg_mask, dim=1) / (
        torch.sum(neg_mask, dim=1) + 1e-12)  # (bsz, )
    E_pos = get_positive_expectation(res * pos_mask, measure, average=False)
...
def batch_video_query_loss_base_semantic(video, query, match_labels, non_match_labels, mask, measure='JSD'):
    """
        QV-CL module
        Computing the Contrastive Loss between the video and query.
        :param video: video rep (bsz, Lv, dim)
    ...
    # generate mask
    pos_mask = match_labels.type(torch.float32)  # (bsz, Lv)
    neg_mask = non_match_labels # (bsz, Lv)
...
# 修改
def fine_grain_batch_video_query_loss(fine_grain_similarity, match_labels, mask, measure='JSD'):
    """
        QV-CL module
        Computing the Contrastive Loss between the video and word_feat.
        :param video: video rep (bsz, Lv, dim)
        :param word_feat: word_feat rep (bsz, dim)
        :param match_labels: match labels (bsz, Lv)
        :param mask: mask (bsz, Lv)
        :param measure: estimator of the mutual information
        :return: L_{qv}
    ...
    # compute scores
    # word_feat = word_feat.unsqueeze(2)  # (bsz, dim, 1)
    res = fine_grain_similarity  # (bsz, Lv)
