...
class BackTrainablePositionalEncoding(nn.Module):
...
class TrainablePositionalEncoding(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, max_position_embeddings, type_vocab_size, hidden_size, dropout=0.1, is_query=False):
        super(TrainablePositionalEncoding, self).__init__()
        self.position_embeddings = nn.Embedding(max_position_embeddings, hidden_size)
    ...
    def forward(self, input_feat, token_type_id):
        """
        Args:
            input_feat: (N, L, D)
        """
        ...
        token_type = torch.full((seq_length,), token_type_id, dtype=torch.long, device=input_feat.device)
        token_type = token_type.unsqueeze(0).repeat(bsz, 1) # (N, L)
